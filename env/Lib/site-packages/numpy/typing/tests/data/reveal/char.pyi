...
assert_type(
    np.char.array(1, unicode=True),
    np.char.chararray[tuple[int, ...], np.dtype[np.str_]],
)

assert_type(
    np.char.asarray(AR_U), np.char.chararray[tuple[int, ...], np.dtype[np.str_]]
)
assert_type(
...
assert_type(
    np.char.asarray("bob"), np.char.chararray[tuple[int, ...], np.dtype[np.str_]]
)
assert_type(
