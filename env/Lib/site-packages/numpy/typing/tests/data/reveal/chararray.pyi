...
assert_type(
    AR_S.center([2, 3, 4], b"a"),
    np.char.chararray[tuple[int, ...], np.dtype[np.bytes_]],
)
...
assert_type(
    AR_S.expandtabs(tabsize=4), np.char.chararray[tuple[int, ...], np.dtype[np.bytes_]]
)

assert_type(AR_U.join("_"), np.char.chararray[tuple[int, ...], np.dtype[np.str_]])
assert_type(
    AR_S.join([b"_", b""]), np.char.chararray[tuple[int, ...], np.dtype[np.bytes_]]
)
...
assert_type(
    AR_S.lstrip(chars=b"_"), np.char.chararray[tuple[int, ...], np.dtype[np.bytes_]]
)
assert_type(AR_U.rstrip(), np.char.chararray[tuple[int, ...], np.dtype[np.str_]])
assert_type(
    AR_S.rstrip(chars=b"_"), np.char.chararray[tuple[int, ...], np.dtype[np.bytes_]]
)
assert_type(AR_U.strip(), np.char.chararray[tuple[int, ...], np.dtype[np.str_]])
assert_type(
    AR_S.strip(chars=b"_"), np.char.chararray[tuple[int, ...], np.dtype[np.bytes_]]
)

assert_type(AR_U.partition("\n"), np.char.chararray[tuple[int, ...], np.dtype[np.str_]])
assert_type(
    AR_S.partition([b"a", b"b", b"c"]),
    np.char.chararray[tuple[int, ...], np.dtype[np.bytes_]],
)
assert_type(
    AR_U.rpartition("\n"), np.char.chararray[tuple[int, ...], np.dtype[np.str_]]
...
assert_type(
    AR_S.rpartition([b"a", b"b", b"c"]),
    np.char.chararray[tuple[int, ...], np.dtype[np.bytes_]],
)

assert_type(
    AR_U.replace("_", "-"), np.char.chararray[tuple[int, ...], np.dtype[np.str_]]
)
assert_type(
...
assert_type(
    AR_S.zfill([2, 3, 4]), np.char.chararray[tuple[int, ...], np.dtype[np.bytes_]]
)
