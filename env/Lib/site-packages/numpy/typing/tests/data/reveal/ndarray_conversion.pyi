...
assert_type(
    cG_4d.item(-0xED_FED_DEB_A_DEAD_BEE), complex
)  # c'mon Ed, we talked about this...
...
assert_type(
    i4_2d.astype(uncertain_dtype),
    np.ndarray[tuple[int, int], np.dtype[np.generic[Any]]],
)
assert_type(
    np.astype(i4_2d, uncertain_dtype), np.ndarray[tuple[int, int], np.dtype[Any]]
)
