...
class TestChandrupatla(TestScalarRootFinders):
    ...
    def test_vectorization(self, shape, xp):
        ...
        assert xp.all(
            (res.x[finite] == res.xl[finite]) | (res.x[finite] == res.xr[finite])
        )
        ...
        assert xp.all(
            xp.abs(res.fun[finite])
            == xp.minimum(xp.abs(res.fl[finite]), xp.abs(res.fr[finite]))
        )
    ...
    def test_nit_expected(self, case, xp):
        ...
        xp_assert_close(
            res.fun, xp.asarray(f(root), dtype=xp.float64), rtol=1e-8, atol=2e-3
        )
