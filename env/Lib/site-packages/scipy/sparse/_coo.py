...
class _coo_base(_data_matrix, _minmax_mixin):
    ...
    def _sparse_dot(self, other):
        ...
        self_raveled_coords = _ravel_non_reduced_axes(
            self.coords, self.shape, [self.ndim - 1]
        )
        other_raveled_coords = _ravel_non_reduced_axes(
            other.coords, other.shape, [other.ndim - 2]
        )
    ...
    def _sparse_tensordot(self, other, axes_self, axes_other):
        ...
        # Ravel non-reduced axes coordinates
        self_non_red_coords = _ravel_non_reduced_axes(
            self.coords, self.shape, axes_self
        )
        self_reduced_coords = np.ravel_multi_index(
            [self.coords[ax] for ax in axes_self], [self.shape[ax] for ax in axes_self]
        )
        other_non_red_coords = _ravel_non_reduced_axes(
            other.coords, other.shape, axes_other
        )
    ...
    def _dense_tensordot(self, other, axes_self, axes_other):
        ...
        non_reduced_axes_other = [
            ax for ax in range(ndim_other) if ax not in axes_other
        ]
