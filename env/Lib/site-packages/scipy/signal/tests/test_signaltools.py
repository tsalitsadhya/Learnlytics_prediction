...
class _TestLinearFilter:
    ...
    def test_scalar_input(self, a, b):
        ...
        assert_allclose(
            lfilter(np.array([1.0]), np.array([1.0]), data), lfilter(b, a, data)
        )
...
@pytest.mark.parametrize(
    "dt", [np.csingle, np.cdouble, pytest.param(np.clongdouble, marks=_pmf)]
)
...
class TestEnvelope:
    ...
    def test_envelope_verify_parameters(self):
        ...
        self.assert_close(
            ze_1**2, ze2_0, msg="Unsquared versus Squared envelope calculation error"
        )
        self.assert_close(
            zr_1, zr_0, msg="Unsquared versus Squared residual calculation error"
        )

        ze2_2, zr_2 = envelope(z, (1, 3), residual="all", squared=True, n_out=3 * n)
        self.assert_close(
            ze2_2[::3], ze2_0, msg="3x up-sampled envelope calculation error"
        )
        self.assert_close(
            zr_2[::3], zr_0, msg="3x up-sampled residual calculation error"
        )

        ze2_3, zr_3 = envelope(z, (1, 3), residual="lowpass", squared=True)
        self.assert_close(
            ze2_3, ze2_0, msg="`residual='lowpass'` envelope calculation error"
        )
        self.assert_close(
            sp_fft.rfft(zr_3),
        ...
        self.assert_close(
            ze2_4, ze2_0, msg="`residual=None` envelope calculation error"
        )
    ...
    def test_envelope_real_signals(self, Z, bp_in, Ze2_desired, Zr_desired):
        ...
        self.assert_close(
            Ze2, Ze2_desired, msg="Envelope calculation error (residual='all')"
        )
        self.assert_close(
            Zr, Zr_desired, msg="Residual calculation error (residual='all')"
        )
        ...
        self.assert_close(
            Zr_lp, Zr_desired, msg="Residual calculation error (residual='lowpass')"
        )
